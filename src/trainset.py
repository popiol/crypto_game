...
class Trainset:
    ...
    def store_shared_input(self, timestamp: datetime, input: np.ndarray):
        with open(self.get_shared_input_file_path(timestamp), "wb") as f:
            pickle.dump(input, f)

    def store_agent_input(self, timestamp: datetime, input: np.ndarray, agent: str):
        with open(self.get_agent_input_file_path(timestamp, agent), "wb") as f:
            pickle.dump(input, f)

    def store_output(self, timestamp: datetime, output: np.ndarray, agent: str):
        with open(self.get_output_file_path(timestamp, agent), "wb") as f:
            pickle.dump(output, f)

    def get_by_timestamp(self, timestamp: datetime, agent: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
